...
import { addDemoModelRequire, queryDemoModelRequire, removeDemoModelRequire, updateDemoModelRequire, queryDemoModelRequireVerboseName, queryDemoModelRequireListDisplay, queryDemoModelRequireDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleAdd = async fields => {
    ...
    try {
      await addDemoModelRequire({ ...fields });
      hide();
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateDemoModelRequire(value, current_id);
      hide();
  ...
  const handleRemove = async selectedRows => {
    ...
    try {
      ...
      await removeDemoModelRequire(ids);
      hide();
  ...
  useEffect(() => {
    queryDemoModelRequireDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryDemoModelRequireListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryDemoModelRequire, table_columns, '下拉选择示例(choices)-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索下拉选择示例(choices)" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '下拉选择示例(choices)-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryDemoModelRequire({ ...params, sorter, filter })}
