...
import { addCategory, queryCategory, removeCategory, updateCategory, queryCategoryVerboseName, queryCategoryListDisplay, queryCategoryDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateCategory(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryCategoryDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryCategoryListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryCategory, table_columns, '分类[被外键关联]-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索分类[被外键关联]" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '分类[被外键关联]-select')
                    }
                  }}
                  selectedKeys={[]}
                >
